...
| UI/COPY | N/A | `/release-notifications/page.tsx` now uses the BRIT black/gold accent treatment on cards, filters, progress, and teammate badges; missing `release_notifications_*` translation keys were added to `LanguageContext` for complete EN/HE title/subtitle/progress text rendering. |
