...
export async function generateMandatePdf(data: MandateData): Promise<Uint8Array> {
  ...
  drawText(
    `1. ${playerName}, born: ${dob}. Nationality: ${nationality}, identification document: passport No. ${passportNo}.`,
    BODY_SIZE,
    false
  );
  y -= 4;
  drawText(
    `2. ${data.agentName} - FIFA Licensed Football Agent (FIFA Football Agent License ID: ${data.fifaLicenseId}), acting through ${AGENCY_NAME}.`,
    BODY_SIZE,
    false
  );
  y -= 4;
  ...
  y -= 4;
  drawText('Valid Leagues in the mandate.', HEADING_SIZE, true);
  y -= 4;
  ...
  const sections = [
    ...
    ['II. EXCLUSIVITY', '3. The Mandate is exclusive. During the Term, the Player shall not appoint, consult, or use any third party, whether licensed or unlicensed, to perform football agent services or to negotiate or facilitate a transaction on the Player\'s behalf.'],
    ...
    ['III. TERM', `5. The Mandate starts on ${effectiveStr} and ends on ${formatDate(data.expiryDate)} (the "Term").`],
    ['IV. SERVICE FEE', '6. In case the club will pay the commission - the commission will be paid to the agent (if there more than 1 agents involved from other agencies, the commission will be paid equally 50-50%.'],
    ['', '7. Payment shall be made to the Agency\'s designated account and is exclusive of VAT, if applicable.'],
    ...
    ['VII. TERMINATION', '16. Either Party may terminate this Mandate for just cause by written notice to the other Party.'],
    ...
    ['VIII. GOVERNING LAW AND ARBITRATION', '19. This Mandate is governed by Swiss law.'],
    ...
    ['IX. INDEPENDENT LEGAL ADVICE', '22. The Player confirms that the Football Agent informed him in writing that he should consider obtaining independent legal advice and that the Player has either obtained such advice or knowingly waived it, as confirmed in the attached annex.'],
    ['X. SIGNATURES', 'A copy of the Agreement has been provided to the Player.'],
  ];
