...
export default function GenerateMandateWomenPage() {
  ...
  if (loading || !user) {
    return (
      <div className="min-h-screen bg-mgsr-dark flex items-center justify-center">
        <div className="animate-pulse text-[var(--women-rose)] font-display">{t('loading')}</div>
      </div>
    );
  }
    ...
    return (
      <AppLayout>
        <div className="max-w-2xl mx-auto py-12 text-center">
          ...
          <Link href={`/players/women/${id}`} className="text-[var(--women-rose)] hover:underline">
  ...
  return (
    <AppLayout>
      <div dir={dir} className="max-w-2xl mx-auto py-4 sm:py-8 px-4">
        <Link
          href={`/players/women/${id}`}
          className={`inline-flex items-center gap-2 text-[var(--women-rose)] hover:underline mb-8 transition-colors ${isRtl ? 'flex-row-reverse' : ''}`}
        >
        ...
        <div dir={dir} className="flex gap-2 mb-8">
          {[0, 1, 2].map((s) => (
            <div
              key={s}
              className={`h-1.5 flex-1 rounded-full transition-colors ${s <= step ? 'bg-[var(--women-rose)]' : 'bg-mgsr-border'}`}
            />
          ))}
        </div>
                  ...
                  <button
                    ...
                    className={`w-full p-4 rounded-xl border-2 text-start transition-all ${
                      selectedAgent?.id === a.id
                        ? 'border-[var(--women-rose)] bg-[var(--women-rose)]/15 shadow-sm'
                        : 'border-mgsr-border hover:border-[var(--women-rose)]/40 hover:bg-mgsr-card/50'
            ...
            <div className="border-t border-mgsr-border pt-5 mt-2">
              ...
              >
                <div className="relative inline-flex items-center">
                  ...
                  />
                  <div className="w-11 h-6 bg-mgsr-border rounded-full peer peer-checked:bg-[var(--women-rose)] transition-colors" />
                  <div className="absolute left-[2px] top-[2px] w-5 h-5 bg-white rounded-full transition-transform peer-checked:translate-x-5" />
                </div>
                <div>
                  <p className={`font-semibold text-sm ${withOriginAgent ? 'text-[var(--women-rose)]' : 'text-mgsr-text'}`}>{t('mandate_with_origin_agent')}</p>
                ...
                <div className="mt-4 space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 rounded-xl bg-mgsr-card border-2 border-mgsr-border text-mgsr-text focus:border-[var(--women-rose)] focus:outline-none transition-colors"
                    />
                  </div>
                  <div>
                        ...
                        <button
                          ...
                          className={`p-3 rounded-xl border-2 text-center text-sm font-semibold transition-all ${
                            originAgentUseLicense === isLicense
                              ? 'border-[var(--women-rose)] bg-[var(--women-rose)]/15 text-[var(--women-rose)]'
                              : 'border-mgsr-border text-mgsr-text hover:border-[var(--women-rose)]/40'
                          }`}
                        >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 rounded-xl bg-mgsr-card border-2 border-mgsr-border text-mgsr-text focus:border-[var(--women-rose)] focus:outline-none transition-colors font-mono"
                    />
                  </div>
                </div>
              )}
        ...
        {step === 1 && (
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="relative rounded-xl bg-mgsr-card border-2 border-mgsr-border focus-within:border-[var(--women-rose)] transition-colors overflow-hidden">
                <div
            ...
            <div>
              ...
              <button
                type="button"
                onClick={openModal}
                className={`w-full py-3 px-4 rounded-xl border-2 border-dashed border-[var(--women-rose)]/50 text-[var(--women-rose)] hover:border-[var(--women-rose)] hover:bg-[var(--women-rose)]/10 transition-all flex items-center justify-center gap-2 ${isRtl ? 'flex-row-reverse' : ''}`}
              >
        ...
        {step === 2 && (
          <div className="space-y-5">
            ...
            <div className="p-6 rounded-2xl bg-mgsr-card border-2 border-mgsr-border space-y-5 shadow-[0_0_30px_rgba(232,160,191,0.08)]">
              <div>
        ...
        <div dir={dir} className={`flex gap-3 mt-8 ${isRtl ? 'justify-start flex-row-reverse' : 'justify-end'}`}>
          ...
          {step < 2 ? (
            <button
              ...
              className="px-6 py-3 rounded-xl bg-[var(--women-rose)] text-mgsr-dark font-semibold hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {t('mandate_next')}
            </button>
          ) : (
            <button
              ...
              disabled={generating}
              className="px-6 py-3 rounded-xl bg-[var(--women-rose)] text-mgsr-dark font-semibold hover:opacity-90 transition disabled:opacity-50 flex items-center gap-2"
            >
              {generating ? (
                <>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 women-dialog-backdrop" onClick={closeModal}>
          <div
            dir={dir}
            className="women-dialog-content bg-mgsr-dark rounded-2xl max-w-md w-full max-h-[90vh] overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="women-dialog-accent" />
            <div className="p-5 border-b border-mgsr-border/50">
              ...
              {!modalSelectedCountry ? (
                <>
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-mgsr-card border-2 border-mgsr-border text-mgsr-text placeholder-mgsr-muted focus:border-[var(--women-rose)] focus:outline-none"
                    autoFocus
                  />
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {filteredCountries.slice(0, 50).map((c) => (
                      <button
                        ...
                        className={`w-full p-3 rounded-xl bg-mgsr-card border border-mgsr-border text-mgsr-text text-start hover:border-[var(--women-rose)]/50 transition-colors ${isRtl ? 'text-right' : 'text-left'}`}
                      >
                        {c}
                      </button>
                    ))}
                ...
                <>
                  <div className="p-3 rounded-xl bg-[var(--women-rose)]/10 border border-[var(--women-rose)]/30 flex items-center justify-between gap-2">
                    <span className="font-medium text-mgsr-text">{modalSelectedCountry}</span>
                    <button
                      type="button"
                      onClick={() => setModalSelectedCountry(null)}
                      className="text-sm text-[var(--women-rose)] hover:underline"
                    >
                  ...
                  <label className={`flex items-center justify-between gap-3 cursor-pointer p-3 rounded-xl border-2 transition-colors ${
                    modalEntireCountry ? 'border-[var(--women-rose)] bg-[var(--women-rose)]/10' : 'border-mgsr-border'
                  }`}>
                    ...
                    <input
                      ...
                      className="rounded border-mgsr-border text-[var(--women-rose)] focus:ring-[var(--women-rose)]"
                    />
                  </label>
                    ...
                    <>
                      ...
                      {modalSearchingClubs && (
                        <div className="flex justify-center py-2">
                          <div className="w-6 h-6 border-2 border-[var(--women-rose)] border-t-transparent rounded-full animate-spin" />
                        </div>
                      )}
                        ...
                        <div className="space-y-2 max-h-40 overflow-y-auto">
                          {modalClubResults.map((club) => (
                            <button
                              ...
                              className={`w-full p-3 rounded-xl bg-mgsr-card border border-mgsr-border flex items-center gap-3 hover:border-[var(--women-rose)]/50 transition-colors ${isRtl ? 'flex-row-reverse' : ''}`}
                            >
                              {club.clubLogo && (
            ...
            <div className="p-5 border-t border-mgsr-border/50">
              <button
                type="button"
                onClick={confirmModalSelection}
                disabled={!canAddInModal}
                className="w-full py-3 rounded-xl bg-[var(--women-gradient)] text-white font-semibold shadow-[0_0_20px_rgba(232,160,191,0.25)] hover:opacity-95 transition disabled:opacity-50 disabled:cursor-not-allowed"
              >
