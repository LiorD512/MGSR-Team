          ...
          {todayBirthdays.map((player) => (
            ...
            >
              ...
              <button
                ...
                disabled={sentWishes.has(player.id)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all shrink-0 ${
                  sentWishes.has(player.id)
                    ? `bg-[${accentColor}]/15 opacity-80 cursor-default`
                    : 'bg-[rgba(37,211,102,0.15)] text-[#25D366] hover:bg-[rgba(37,211,102,0.3)]'
                }`}
                style={sentWishes.has(player.id) ? { color: accentColor } : undefined}
              >
                {sentWishes.has(player.id) ? (
                  <>
