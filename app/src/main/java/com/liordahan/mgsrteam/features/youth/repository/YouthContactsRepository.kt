...
/**
 * Youth-dedicated contacts repository.
 * Hardcoded to "ContactsYouth" collection — no PlatformManager dependency.
 */
class YouthContactsRepository(
    private val firebaseHandler: YouthFirebaseHandler
) {

    fun contactsFlow(): Flow<List<YouthContact>> = callbackFlow {
        val listener: ListenerRegistration = firebaseHandler.firebaseStore
            ...
            .addSnapshotListener { value, error ->
                ...
                val list = value?.toObjects(YouthContact::class.java) ?: emptyList()
                trySend(list.sortedBy { it.name?.lowercase() })
            }
        awaitClose { listener.remove() }
    }

    suspend fun addContact(contact: YouthContact): Result<Unit> = runCatching {
        val data = mapOf(
            ...
            "contactType" to (contact.contactType ?: YouthContactType.CLUB.name),
    ...
    suspend fun updateContact(contact: YouthContact): Result<Unit> = runCatching {
