...
/**
 * Women-dedicated requests repository.
 * Hardcoded to "ClubRequestsWomen" collection — no PlatformManager dependency.
 */
class WomenRequestsRepository(
    private val firebaseHandler: WomenFirebaseHandler
) {

    fun requestsFlow(): Flow<List<WomenRequest>> = callbackFlow {
        val listener: ListenerRegistration = firebaseHandler.firebaseStore
            ...
            .addSnapshotListener { value, error ->
                ...
                val list = value?.documents?.mapNotNull { doc ->
                    doc.toObject(WomenRequest::class.java)?.copy(id = doc.id)
                } ?: emptyList()
    ...
    private fun writeFeedEvent(type: String, request: WomenRequest, agentName: String?) {
        try {
    ...
    suspend fun addRequest(request: WomenRequest): Result<Unit> = runCatching {
        ...
        firebaseHandler.firebaseStore
            ...
            .add(data)
            .await()
        writeFeedEvent(WomenFeedEvent.TYPE_REQUEST_ADDED, request, agentName)
    }

    suspend fun updateRequest(request: WomenRequest): Result<Unit> = runCatching {
    ...
    suspend fun deleteRequest(request: WomenRequest): Result<Unit> = runCatching {
        val id = request.id ?: throw IllegalArgumentException("Request id required for delete")
        val agentName = getCurrentUserAccountName() ?: FirebaseAuth.getInstance().currentUser?.displayName
        firebaseHandler.firebaseStore
            .collection(firebaseHandler.clubRequestsTable)
            .document(id)
            .delete()
            .await()
        writeFeedEvent(WomenFeedEvent.TYPE_REQUEST_DELETED, request, agentName)
    }
}
